...
export_folder_name = "edgelists_temp"
...
def normalize(word):
    ...
    if not word.isalnum() and not bool(re.match("^.*ÃƒÆ’Ã‚Â±.*$", word)):
...
def create_edgelist(df):
    ...
    if stopwords_tl == None or stopwords_en == None:
        ...
        return df.reset_index(drop=True)[['text', 'word_list', 'source', 'source_type', 'year']]
